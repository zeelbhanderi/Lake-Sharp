...
class Parser:
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Expected 'WETURN', 'CONTINUWU', 'BWEAK', 'MADEWATER', 'IF', 'FOR', 'WHILE', 'FWUNCTION', int, float, identifier, '+', '-', '(', '[' or 'NOT'",
                )
            )
    ...
    def expr(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, "MADEWATER"):
            res.register_advancement()
            self.advance()
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Expwected 'MADEWATER', 'IF', 'FOR', 'WHILE', 'FWUNCTION', int, float, identifier, '+', '-', '(', '[' or 'NOT'",
                )
            )
    ...
    def if_expr_cases(self, case_keyword):
        ...
        if not self.current_tok.matches(TT_KEYWORD, "INOUTWATER"):
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    f"Expwected 'INOUTWATER'",
                )
            )
